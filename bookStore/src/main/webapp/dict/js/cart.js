...
function changeNum() {
	...
	for(var i = 0; i < trs.length; i++) {
		...
		td.onclick = function() {
			var reg = /^[0-9]+$/;
			td1 = this.parentNode.getElementsByTagName("td")[5];
			td2 = this.parentNode.getElementsByTagName("td")[2];
			td3 = this.parentNode.getElementsByTagName("td")[4];
			td4 = this.parentNode.getElementsByTagName("td")[7];
				  ...
				  if(xmlhttp.readyState==4)
				      {
				       ...
				       var info=s.split(":");
				       var info1=info[1]-0;
				       var sinprice=td1.innerText;
				       var allprice=td1.innerText.substring(0,sinprice.length-1)*n;
				       allprice=num(allprice);
				       td4.innerText=allprice;
				       info1=num(info1);
				       totalprice.innerText=info1;
				       aprice.innerText=info1;
				       acount.innerText=info[0];
				                 }
				                }
				    }
				      xmlhttp.open("get","/BookStore/cart/addnum?bisbn="+td2.innerText+"&shopcount="+n+"&bname="+td3.innerText,true);
				      xmlhttp.send();
			...
			xmlhttp.onreadystatechange= function () {
			  if(xmlhttp.readyState==4)
			      ...
			      var info=s.split(":");
			       var info2=info[1]-0;
			       info2=num(info2);
			       totalprice.innerText=info2;
			       aprice.innerText=info2;
			       acount.innerText=info[0];
			                 }
			                }
			    }
			      xmlhttp.open("get","/BookStore/cart/del?bisbn="+info[0]+"&bname="+info[1],true);
			      xmlhttp.send();
		}
	}
	//调用关闭按钮的单击事件
