 ...
 */
@WebServlet(urlPatterns="/tejia/list")
public class TejiaListAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        PageBean<Book> pageBean=ibook.queryByCondition(page, 4, null,3l);
        if(page>pageBean.getPageCounts())
        {
            page=pageBean.getPageCounts();
            pageBean=ibook.queryByCondition(page, 4, null,3l);
        }
        request.setAttribute("pageBean", pageBean);
        request.setAttribute("bookStyle", bookStyle);
        request.getRequestDispatcher("/jsp/book/pageBean3.jsp").forward(request, response);
    }
