 ...
 */
@WebServlet(urlPatterns="/tuijian/list")
public class TuijianListAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	       ...
	       String pageNow = request.getParameter("pageNow");
	        ...
	        PageBean<Book> pageBean=ibook.queryByCondition(page, 4, null,2l);
	        if(page>pageBean.getPageCounts())
	        {
	            page=pageBean.getPageCounts();
	            pageBean=ibook.queryByCondition(page, 4, null,2l);
	        }
	        request.setAttribute("pageBean", pageBean);
	        request.setAttribute("bookStyle", bookStyle);
	        request.getRequestDispatcher("/jsp/book/pageBean2.jsp").forward(request, response);
	    }
